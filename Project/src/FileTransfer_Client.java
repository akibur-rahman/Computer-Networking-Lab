...
public class FileTransfer_Client {
				...
				if (i == 1) {
					...
					while (!(j == 4)) {
						...
						if (j == 1) {
							...
							} while (!(input_serial_no <= files_length && input_serial_no >= 0)); // loops until proper
																									// input is
																									// submitted
	...
	private static void receiveFile() {
		...
		try {
			...
			// fileFoundFlag is a Flag which denotes the file is present or absent from the
			// Server directory, is present int 0 is sent, else 1
			...
			bytesRead = bis.read(byteArray, 0, byteArray.length); // reads bytes of length byteArray from
																	// BufferedInputStream & writes into the byteArray,
																	// (Offset 0 and length is of byteArray)
			current = bytesRead;

			// Sometimes only a portion of the file is read, hence to read the remaining
			// portion...
			do {
				// BufferedInputStream is read again into the byteArray, offset is current
				// (which is the amount of bytes read previously) and length is the empty space
				// in the byteArray after current is subtracted from its length
			...
			bos.write(byteArray, 0, current); // writes bytes from the byteArray into the BufferedOutputStream, offset
												// is 0 and length is current (which is the amount of bytes read into
												// byteArray)

			bos.close();

			System.out.println("        File " + fileName + " Successfully Downloaded!");
			dout.writeInt(0); // writeInt is used to reset if any bytes are present in the buffer after the
								// file transfer
		} catch (IOException ex) {
			ex.printStackTrace();
		}
